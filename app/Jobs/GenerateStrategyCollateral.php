    ...
    private function generateCollateral(): void
    {
        ...
        if ($this->hasActionableVideoContent($videoStrategy)) {
            ...
            if ($firstStrategyId === $this->strategy->id) {
                ...
                GenerateVideo::dispatch($this->campaign, $this->strategy, 'Google Ads (Performance Max)')
                    ->delay(now()->addSeconds(120 + $strategySpread));
                // Portrait (9:16) — Meta / Facebook
                GenerateVideo::dispatch($this->campaign, $this->strategy, 'Facebook Ads')
                    ->delay(now()->addSeconds(120 + $strategySpread + 240));
                Log::info("Dispatched 2 campaign-level videos (landscape + portrait) for Campaign ID: {$this->campaign->id}, Strategy ID: {$this->strategy->id}");
            } else {

...
class ExtendVideo implements ShouldQueue
{
    ...
    public function handle(GeminiService $geminiService): void
    {
        ...
        try {
            ...
            // Use the original Veo generation URI stored in the DB.
            // Re-uploading the video bytes to the Files API produces a new URI that Veo
            // does not recognise as a Veo-generated video, causing a 400 INVALID_ARGUMENT:
            // "Input video must be a video that was generated by VEO that has been processed."
            //
            // The stored gemini_video_uri is the full download URL returned by Veo at
            // generation time (e.g. https://generativelanguage.googleapis.com/v1beta/files/xxx:download?alt=media).
            // The Veo extension API expects the short resource name "files/{file_id}" — not the
            // full URL. Extract that short name from the stored URI.
            $rawUri = $this->sourceVideo->gemini_video_uri;
            if (preg_match('/\/(files\/[^:?\/]+)/', $rawUri, $matches)) {
                $videoUri = $matches[1]; // "files/{file_id}"
            } else {
                // Fallback: strip download suffix from whatever we have
                $videoUri = preg_replace('/:download(\?alt=media)?$/', '', $rawUri);
            }
            Log::info("ExtendVideo: Using Veo generation URI for extension", [
                'raw_uri' => $rawUri,
                'uri'     => $videoUri,
            ]);

<?php

namespace Database\Seeders;

use App\Models\Plan;
use Illuminate\Database\Seeder;

class PlanSeeder extends Seeder
{
    public function run(): void
    {
        Plan::updateOrCreate(
            ['slug' => 'free'],
            [
                'name' => 'Free',
                'description' => 'Explore the platform with generous limits.',
                'price_cents' => 0,
                'billing_interval' => 'month',
                'stripe_price_id' => null,
                'features' => [
                    'Google Ads Only',
                    '1 Campaign',
                    '1 Brand Identity (Vision AI Extraction)',
                    '4 AI-Generated Images per Campaign',
                    '3 Landing Page CRO Audits',
                    'Unlimited Ad Copy Generation',
                    'Analytics Dashboard',
                ],
                'is_active' => true,
                'is_free' => true,
                'is_popular' => false,
                'cta_text' => 'Get Started Free',
                'sort_order' => 0,
                'creative_limits' => [
                    'image_generations' => 4,
                    'video_generations' => 0,
                    'refinements' => 0,
                    'max_refinements_per_item' => 0,
                    'max_extensions_per_video' => 0,
                ],
            ]
        );

        Plan::updateOrCreate(
            ['slug' => 'starter'],
            [
                'name' => 'Starter',
                'description' => 'For local businesses and early-stage startups.',
                'price_cents' => 14900,
                'billing_interval' => 'month',
                'stripe_price_id' => 'price_1Tfzd0ClfeR0n0yLuYUEUg6D',
                'features' => [
                    '1 Ad Platform — your choice of Google or Facebook',
                    'Unlimited Campaigns',
                    '3 Brand Identities',
                    'AI Ad Copy & Image Generation',
                    'Automatic Ad Fixing & Budget Shifting',
                    'Analytics Dashboard',
                    'Basic Email Support',
                ],
                'is_active' => true,
                'is_free' => false,
                'is_popular' => false,
                'cta_text' => 'Start Free Trial',
                'badge_text' => null,
                'sort_order' => 1,
                'creative_limits' => [
                    'image_generations' => 50,
                    'video_generations' => 10,
                    'refinements' => 50,
                    'max_refinements_per_item' => 3,
                    'max_extensions_per_video' => 3,
                ],
            ]
        );

        Plan::updateOrCreate(
            ['slug' => 'growth'],
            [
                'name' => 'Growth',
                'description' => 'For established businesses spending $1,000+/mo on Google Ads.',
                'price_cents' => 24900,
                'billing_interval' => 'month',
                'stripe_price_id' => 'price_1TFYpAClfeR0n0yLugIalDeU',
                'features' => [
                    'Everything in Starter, plus:',
                    'All 4 Platforms (Google, Facebook, Microsoft, LinkedIn)',
                    'Unlimited Brand Identities',
                    'Competitor Discovery & Analysis',
                    'Video & Carousel Creative',
                    'Daily Optimisation',
                    'War Room Strategy Access',
                    'Priority Support',
                ],
                'is_active' => true,
                'is_free' => false,
                'is_popular' => true,
                'cta_text' => 'Start Scaling Now',
                'badge_text' => 'MOST POPULAR',
                'sort_order' => 2,
                'creative_limits' => [
                    'image_generations' => 150,
                    'video_generations' => 30,
                    'refinements' => 150,
                    'max_refinements_per_item' => 3,
                    'max_extensions_per_video' => 3,
                ],
            ]
        );

        Plan::updateOrCreate(
            ['slug' => 'agency'],
            [
                'name' => 'Agency',
                'description' => 'For high-volume advertisers and marketing agencies.',
                'price_cents' => 0,
                'billing_interval' => 'month',
                'stripe_price_id' => null,
                'features' => [
                    'Everything in Growth, plus:',
                    'Multi-Client Management (10 sub-accounts)',
                    'White-Label Reports',
                    'Dedicated Account Success Manager',
                    'Early Access to New Features',
                ],
                'is_active' => true,
                'is_free' => false,
                'is_popular' => false,
                'cta_text' => 'Contact Sales',
                'badge_text' => null,
                'sort_order' => 3,
                'creative_limits' => null,
            ]
        );
    }
}

        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5">
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                    {c.discovery_source && (
                        <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full capitalize">{c.discovery_source.replace(/_/g, ' ')}</span>
                    )}
                    {c.last_analyzed_at && (
                        <span className="text-xs text-gray-500">Analyzed {new Date(c.last_analyzed_at).toLocaleDateString()}</span>
                    )}
                </div>
            </div>
                        ...
                        <div className="space-y-1">
                            {c.messaging_analysis.competition_type && (
                                <p className="text-xs text-gray-500">
                                    ...
                                    {c.messaging_analysis.estimated_size && <span className="text-gray-500"> · {c.messaging_analysis.estimated_size}</span>}
                                </p>
                            )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Generated by competitive intelligence agent</p>
                </div>
                {updatedAt && (
                    <span className="text-xs text-gray-500">Updated {new Date(updatedAt).toLocaleDateString()}</span>
                )}
            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {strategy.ad_copy_examples.slice(0, 4).map((ad, i) => (
                            <div key={i} className="bg-gray-50 rounded-lg p-3">
                                ...
                                {ad.competitor && <p className="text-xs text-gray-500 mt-1">Counter: {ad.competitor}</p>}
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="mx-auto max-w-5xl">
                    ...
                    <div className="flex items-center justify-between mb-1">
                        ...
                        {canAccessCompetitors && (
                            <div className="flex items-center gap-3">
                                {lastAnalyzedAt && (
                                    <span className="text-xs text-gray-500">Last run: {new Date(lastAnalyzedAt).toLocaleDateString()}</span>
                                )}
                                <button

...
describe('Pricing one-time setup card', () => {
    ...
    it('offers the one-time setup with its USD price and starts checkout', () => {
        const { getByText } = render(
            <Pricing auth={auth} plans={[]} journey={{ checkout_ready: true }} setupFee={{ price_usd: 999, intent: false, paid: false }} />
        );
        ...
        expect(router.post).toHaveBeenCalledWith('/__route__/setup-fee.checkout', {}, expect.objectContaining({ onFinish: expect.any(Function) }));
    });
    ...
    it('requires business readiness before offering checkout', () => {
        const { getByText, queryByText } = render(<Pricing auth={auth} plans={[]} journey={{ checkout_ready: false }} setupFee={{ price_usd: 999, intent: true, paid: false }} />);
        expect(getByText('Confirm business details first')).toBeInTheDocument();
        expect(queryByText('Pay setup fee')).toBeNull();

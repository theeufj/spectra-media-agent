 ...
 * the side-by-side table, recent conversions and the touchpoint journey.
 *
 * Pages/Analytics/Attribution and Pages/Campaigns/Attribution were ~340
 * byte-identical lines of this, differing only in the shell around it — so a
 * fix to the shared part (the channel bar, a model label, the currency format)
 * had to be made twice or it was made in one place and silently missing from
 * the other. The two pages now own only their own header and empty-state copy
 * and render this.
 */
...
/**
 * @param {object} props
 * @param {{total_conversions: number, total_value: number, avg_touchpoints: number}} props.summary
 * @param {Record<string, Array<{channel: string, conversions: number, value: number}>>} props.channelBreakdown
 * @param {Array<object>} props.recentTouchpoints
 * @param {Array<object>} props.conversions
 * @param {React.ReactNode} [props.emptyState] Copy shown under the "no data yet"
 *   heading. The campaign page adds the pixel snippet here; the account-wide
...
export default function AttributionReport({ summary, channelBreakdown, recentTouchpoints, conversions, emptyState = null }) {
    return (
        <>
            ...
            {/* Touchpoints are worth showing even before the first conversion —
                they are the proof the pixel is firing at all. */}

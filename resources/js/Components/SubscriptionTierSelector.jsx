...
const SubscriptionTierSelector = ({ plans = [] }) => {
    ...
    return (
        <div>
            ...
            <div className={`grid grid-cols-1 ${plans.length > 1 ? 'md:grid-cols-2' : ''} ${plans.length > 2 ? 'lg:grid-cols-3' : ''} gap-8 max-w-4xl mx-auto`}>
                {plans.map(plan => (
                    <Card key={plan.id} className="flex flex-col">
                        <h3 className="text-xl font-semibold">{plan.name}</h3>
                        <p className="text-3xl font-bold my-4">{plan.formatted_price}</p>

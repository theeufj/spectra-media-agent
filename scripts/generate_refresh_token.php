...
if (! $ini || ! isset($ini['OAUTH2'])) {
    exit("Error: Could not parse google_ads_php.ini or OAUTH2 section missing.\n");
}
...
if ($clientId === 'INSERT_CLIENT_ID_HERE' || $clientSecret === 'INSERT_CLIENT_SECRET_HERE') {
    exit("Error: Please update spectra/storage/app/google_ads_php.ini with your Client ID and Client Secret.\n");
}
...
if (! $server) {
    exit("Error: Could not start local server: $errstr ($errno)\n");
}

$conn = stream_socket_accept($server, 300); // wait up to 5 minutes
if (! $conn) {
    fclose($server);
    exit("Error: Timed out waiting for OAuth callback.\n");
}
...
if (preg_match('/[?&]code=([^\s&]+)/', $request, $matches)) {
    ...
    exit("Error: Could not extract authorization code from callback.\nRequest: $request\n");
}
